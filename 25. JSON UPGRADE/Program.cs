...
class Info
{
    ...
    public Info(string name, string relationship, string phoneNumber = "0000000000", string address = "Việt Nam")
    {
        Name = name;
        PhoneNumber = string.IsNullOrEmpty(phoneNumber) ? "0000000000" : phoneNumber;
        Address = string.IsNullOrEmpty(address) ? "Việt Nam" : address;
...
class Contact
{
    ...
    public List<Info> GetContact(List<Info> newInform, string filePath)
    {
        ...
        {
            Console.WriteLine("Địa chỉ nhập vào bị trống hoặc sai cú pháp, vui lòng nhập lại ");
            address = Console.ReadLine();
        }
        Console.WriteLine("Bạn có muốn đặt mối quan hệ cho liên lạc này không? (Bấm 1 = Có, Phím bất kỳ = Không)");
        string? option = Console.ReadLine();
        if (option == "1")
        {
            Console.WriteLine("");
            Console.WriteLine("Hãy chọn 1 trong 4 mối quan hệ sau: ");
            Console.WriteLine("(1) Bạn bè (friend)");
            Console.WriteLine("(2) Người thân (family)");
            Console.WriteLine("(3) Bạn học (classmate)");
            Console.WriteLine("(4) Đồng nghiệp (colleague)");
            Console.WriteLine("(5) Khách hàng/đối tác(customer)");
            Console.WriteLine("(6) Nhóm khác(others)");
            Console.WriteLine("");
            string? relationshipOption = Console.ReadLine();
            if (relationshipOption == "1")
            {
                relationship = "Bạn bè (friend)";
            }
            else if (relationshipOption == "2")
            {
                relationship = "Người thân (family)";
            }
            else if (relationshipOption == "3")
            {
                relationship = "Bạn học (classmate)";
            }
            else if (relationshipOption == "4")
            {
                relationship = "Đồng nghiệp (colleague)";
            }
            else if (relationshipOption == "5")
            {
                relationship = "Khách hàng/Đối tác (customer)";
            }
            else if (relationshipOption == "6")
            {
                relationship = "Nhóm khác (others)";
            }
            else
            {
        ...
        newInform.Add(new Info (name, relationship, phoneNumber, address));       
        return newInform;
    }
    ...
    public bool IsValidRelationOption(string?  relationOption)
    {
        if (string.IsNullOrEmpty(relationOption))
        {
            return false;
        }
        return true;
    ...
    public List<Info> CheckAndUpdateInfo(List<Info> information, string phoneNumber, string filePath)
    {
        ...
        if (contact != null)
        {
            ...
            Console.WriteLine($"Relationship : {contact.Relationship}");
            Console.WriteLine("Bạn có muốn chỉnh sửa thông tin không? (Bấm 1 = Có, bấm phím bất kỳ = Không)");
            ...
            if (choice == "1")
            {
                ...
                Console.WriteLine("Bạn có muốn đặt lại mối quan hệ cho liên lạc này không? (Bấm 1 = Có, Phím bất kỳ = Không)");
                ...
                if (option3 == "1")
                {
                    ...
                    if (relationshipOption == "1")
                    {
                        newRelationship = "Bạn bè (friend)";
                    }
                    else if (relationshipOption == "2")
                    {
                        newRelationship = "Người thân (family)";
                    }
                    else if (relationshipOption == "3")
                    {
                        newRelationship = "Bạn học (classmate)";
                    }
                    else if (relationshipOption == "4")
                    {
                        newRelationship = "Đồng nghiệp (colleague)";
                    }
                    else if (relationshipOption == "5")
                    {
                        newRelationship = "Khách hàng/Đối tác (customer)";
                    }
                    else if (relationshipOption == "6")
                    {
                        newRelationship = "Nhóm khác (others)";
                    }
                    else
                    {
                        newRelationship = "Không có";
                    }
                }
                else
                {
    ...
    public void FindAndDisplaySortedInfo(List<Info> information, string filePath)
    {
        if (information != null && information.Count > 0)
        {
            foreach (var info in information)
            {
                ...
                Console.WriteLine($"Relationship: {info.Relationship}");
                Console.WriteLine("");
            }
        }
...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            Console.WriteLine("(2). Tìm và sửa thông tin liên lạc dựa trên số điện thoại và địa chỉ gmail");
            Console.WriteLine("(3). Tìm và xóa thông tin liên lạc dựa trên số điện thoại và địa chỉ gmail");
            Console.WriteLine("(4). Tìm thông tin người dùng dựa trên số điện thoại và địa chỉ gmail");
